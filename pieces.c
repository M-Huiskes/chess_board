...
uint64_t find_possible_pawn_moves(
    Piece *piece, Square input_square, int position, uint64_t full_board, GameState *game_state)
{
    ...
    if (piece->symbol == 'P')
    {
        if (input_square.row < 8)
        {
            ...
            if (input_square.row == 1)
            {
                ...
                if (!(is_bit_set(full_board, (position + 16))) && !(is_bit_set(full_board, (position + 8))))
                {
            ...
            if (input_square.row == 4)
            {
                ...
                if (game_state->last_moved_piece == 'p' &&
                    abs(input_square.file - (game_state->last_move[2] - FILE_OFFSET)) == 1 &&
...
uint64_t find_possible_bishop_moves(Piece *piece, int position, uint64_t full_board)
{
    ...
    uint64_t possible_moves = (uint64_t)0;
    find_diagonal_moves(
        position, full_board, &possible_moves, piece, max_counter);
    return possible_moves;
}
...
uint64_t
find_possible_knight_moves(Piece *piece, int position, uint64_t full_board)
{
    uint64_t possible_moves = (uint64_t)0;
    int ver_dir[2] = {-8, 8};
    int hor_step[2] = {-1, 1};
...
uint64_t
find_possible_king_moves(Piece *piece, int position, uint64_t full_board)
{
    // Implement castling
    int max_counter = 1;
    uint64_t possible_moves = (uint64_t)0;
...
uint64_t find_possible_moves(Square input_square, int position, Piece *piece, GameState *game_state)
{
    ...
    case 'p':
        possible_moves =
            find_possible_pawn_moves(piece, input_square, position, full_board, game_state);
        break;
    case 'B':
    case 'b':
        possible_moves = find_possible_bishop_moves(piece, position, full_board);
        break;
    case 'R':
    case 'r':
    ...
    case 'N':
    case 'n':
        possible_moves = find_possible_knight_moves(piece, position, full_board);
        break;
    case 'K':
    case 'k':
